...
kafka-topics --command-config playground.config --bootstrap-server <my-boostrap-server> --topic second_topic --create --partitions 3

# consuming
kafka-console-consumer --consumer.config playground.config --bootstrap-server <my-boostrap-server> --topic second_topic

# other terminal
# round robin partitioner class, we produce to one partition at a time
# only necessary for demo, not in prod
kafka-console-producer --producer.config playground.config --bootstrap-server <my-boostrap-server> --producer-property partitioner.class=org.apache.kafka.clients.producer.RoundRobinPartitioner --topic second_topic

# consuming from beginning
kafka-console-consumer --consumer.config playground.config --bootstrap-server <my-boostrap-server> --topic second_topic --from-beginning

# display key, values and timestamp in consumer
kafka-console-consumer --consumer.config playground.config --bootstrap-server <my-boostrap-server> --topic second_topic --formatter kafka.tools.DefaultMessageFormatter --property print.timestamp=true --property print.key=true --property print.value=true --property print.partition=true --from-beginning
